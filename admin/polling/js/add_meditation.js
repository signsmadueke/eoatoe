...
(function () {
    document.getElementById("meditation_form").addEventListener("submit", function (e) {
        e.preventDefault();
        ...
        fetch(`polling/php/add_meditation`, {
            method: "post",
            body: form_inputs
        }).then(d => d).then(d => d.text()).then(d => {
            if (d === "true") {
                toastr.success("New Meditation Added!", "Meditation Added");
                document.getElementById("meditation_form").reset();
