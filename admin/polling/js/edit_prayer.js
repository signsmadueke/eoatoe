...
(function () {
    document.getElementById("edit_prayer").addEventListener("submit", function (e) {
        e.preventDefault();
        ...
        let prayer_id = document.getElementById("prayerId").dataset.id;
        let form_inputs = new FormData(e.currentTarget);

        fetch(`polling/php/edit?prayer_id=${prayer_id}`, {
            method: "post",
            body: form_inputs
        }).then(d => d).then(d => d.text()).then(d => {
            if (d === "true") {
                toastr.success("Prayer edited successfully!", "Prayer Edited");
                document.getElementById("submit_btn").innerHTML = "<i class='icon-note'></i> Edit Prayer";
