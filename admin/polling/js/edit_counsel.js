...
(function () {
    document.getElementById("edit_counsel").addEventListener("submit", function (e) {
        e.preventDefault();
        ...
        let counsel_id = document.getElementById("counselId").dataset.id;
        let form_inputs = new FormData(e.currentTarget);

        fetch(`polling/php/edit?counsel_id=${counsel_id}`, {
            method: "post",
            body: form_inputs
        }).then(d => d).then(d => d.text()).then(d => {
            if (d === "true") {
                toastr.success("Counsel edited successfully!", "Counsel Edited");
                document.getElementById("submit_btn").innerHTML = "<i class='icon-note'></i> Edit Counsel";
